...
const commands = {
  start: () => Promise.resolve(require('./cli-start.js')),
  dev: () => Promise.resolve(require('./cli-dev.js')),
  build: () => Promise.resolve(require('./cli-build.js')),
  generate: () => Promise.resolve(require('./cli-generate.js')),
  help: () => Promise.resolve(require('./cli-help.js'))
};
...
async function _import(modulePath) {
  for (const mp of [
    ...
    try {
      return await Promise.resolve(require(mp))
    } catch (e) {
...
const tsNode = () => _import('ts-node');
const nuxtTypescript = () => _import('@nuxt/typescript');
...
async function loadNuxtConfig(argv) {
  ...
  try {
    ...
    if (e.code !== 'MODULE_NOT_FOUND') {
      throw (e)
    } else if (argv['config-file'] !== config.defaultNuxtConfigFile) {
  ...
  if (nuxtConfigFile) {
    // Clear cache
    utils.clearRequireCache(nuxtConfigFile);
    ...
    options._nuxtConfigFiles = Array.from(utils.scanRequireTree(nuxtConfigFile));
...
async function detectTypeScript(rootDir, options = {}) {
  const typescript = {
